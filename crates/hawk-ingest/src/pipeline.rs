...
use crate::{
    batch_updater::{apply_batch, BatchReport},
    column_mapper::{map_row, validate_mapping, IngestMapping},
    csv_reader::read_csv_rows,
    schema_inference::{identity_mapping, infer_schema, InferConfig},
};
    ...
    fn read_rows(path: &Path) -> Result<Vec<serde_json::Map<String, serde_json::Value>>> {
        ...
        match extension.as_str() {
            ...
            "json" | "jsonl" => {
                #[cfg(feature = "json")]
                {
                    crate::json_reader::read_json_lines_rows(path)
                }
            ...
            "parquet" => {
                #[cfg(feature = "parquet")]
                {
                    crate::parquet_reader::read_parquet_rows(path)
                }
