...
use crate::parser::{DimRef, ExportFormat, Statement};
...
fn build_dim_key(reference: &DimRef, filters: &[DimRef]) -> hawk_core::DimensionKey {
    ...
    hawk_core::dimension_key_from_pairs(pairs)
}
...
fn exec_show(
    ...
    match &dist.repr {
        hawk_core::DistributionRepr::Categorical {
        ...
        hawk_core::DistributionRepr::Histogram {
...
fn exec_nearest(
    ...
    let parsed = hawk_query::parser::parse_reference(&reference.to_ref_string())
...
fn exec_schema(db: &Database) -> Result<QueryResult> {
    ...
    for v in &schema.variables {
        let desc = match &v.var_type {
            ...
            hawk_core::VariableType::Categorical { categories, allow_unknown } => {
                format!(
